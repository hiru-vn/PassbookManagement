...
namespace DAO
{
    public class ReportDAO
    {
        ...
        public DataTable GetMonthlyReport(int month, int year, int typeid)
        {
            ...
            string query = "select ROW_NUMBER() over(order by typename),typename,collect,withdraw, abs(collect-withdraw) as ' chenh lech' from (select typepassbook.id as idc,case when collectmoney1 is null then 0 else collectmoney1 end as collect,case when withdrawmoney1 is null then 0 else withdrawmoney1 end as 'withdraw' from (select id1, collectmoney1, withdrawmoney1 from (select typepassbook.id as 'id1',sum(collectmoney) 'collectmoney1' from dbo.collectbill, dbo.typepassbook, dbo.passbook where month(collectdate)=" + month + " and year(collectdate)=" + year + " and passbook.id=collect_passbook and passbook_type=typepassbook.id group by typepassbook.id) as a full join (select typepassbook.id 'id2',sum(withdrawmoney) 'withdrawmoney1' from dbo.withdrawbill, dbo.typepassbook, dbo.passbook where month(withdrawdate)=" + month + " and year(withdrawdate)=" + year + " and passbook.id=withdraw_passbook and passbook_type=typepassbook.id group by typepassbook.id) as b on (id1=id2)) c full join dbo.typepassbook on (typepassbook.id=id1)) as d, dbo.typepassbook where idc=typepassbook.id and typepassbook.id=" + typeid;
            DataTable data = DataProvider.Instance.ExcuteQuery(query);
            return data;
        }
    }
