...
declare @balance money;
	select @balance= (select top 1 passbook_balance from inserted)
	declare @status int
	select @status=(select top 1 status from inserted)
	declare @id int
	select @id=(select top 1 id from inserted)
	declare @check int
	select @check = (select top 1 count(*) from withdrawbill where withdraw_passbook=@id)
	if(@check=0)
		begin
		if(@balance!=0)
			begin
			if(@balance <(select top 1 min_passbookbalance from typepassbook, inserted where inserted.passbook_type=typepassbook.id)  
			and @status=0)
				begin 
...
create trigger TG_InsertCollectbill on collectbill
for insert
as
begin
declare @money money
...
create trigger TG_change_id_collectbill on collectbill
for insert
as
begin
declare @max int
...
go
--Trigger tự tạo các thuộc tính của loại tiết kiệm
...
go
--Trigger update loại tiết kiệm
...
go
--Trigger tạo ngày được rút/gởitiền
...
go
--Trigger tạo phiếu rút tiền
create trigger TG_Insert_Withdrawbill on withdrawbill
for insert
as
begin
declare @id int
...
go
--Hàm tổng số tiền gởi trong ngày theo Idtypepassbook
...
go
--Hàm tổng số tiền rút trong ngày theo Idtypepassbook
...
begin
	...
	update dbo.passbook set increasement_date = DATEADD(month,1,increasement_date) where id in (select id from dbo.passbook as p where p.increasement_date <= GETDATE())
end
select * from passbook
go
--ham tong so tien goi theo thang
...
go
--ham tong so tien chi theo thang
...
go
create function reportTypeOpenMonth
(@id int,
@month int, @year int)
returns int
as
begin
declare @value int
select @value = (select count(*) from dbo.typepassbook as t,dbo.passbook as p where p.passbook_type = t.id and  t.id = @id and  month(p.opendate) = @month and year(p.opendate) = @year)
return @value
end
go
create function reportTypeCloseMonth
(@id int,
@month int, @year int)
returns int
as
begin
declare @value int
select @value = (select count(*) from dbo.typepassbook as t,dbo.passbook as p where p.passbook_type = t.id and  t.id = @id and  month(p.withdrawday) = @month and year(p.withdrawday) = @year)
return @value
end
go
...
END
go
--bieu do tien thu/chi theo thang
create proc usp_ReportChartTypePassbookDay 
@month int,
@year int,
@typeid int
as
begin
select 
ISNULL(dbo.reportCM(@typeid,@month,@year), 0) Collect,
ISNULL(dbo.reportWM(@typeid,@month,@year), 0) Withdraw 
end
go
--bieu do so tao/ so dong theo thang
create proc usp_ReportChartTypePassbookMonth 
@month int,
@year int,
@typeid int
as
begin
	select 
	dbo.reportTypeOpenMonth(@typeid,@month,@year) openP,
	dbo.reportTypeCloseMonth(@typeid,@month,@year) closeP 
end
go
--thong ke so tao/ so dong theo thang
...
begin
	...
	select @lastdate = dateadd(day,-1,dateadd(month,1,@startdate))
...
go
--Hàm tìm ngày hoạt động cuối cùng
...
go
--Proc report doanh thu các loại tk
...
go
--Proc insert passbook không nhập ngày
